...
void parseAndExecuteCommand(String json) {
  ...
  String resp;
  if (jamOccurred) {
    resp = "{\"status\":\"ERROR\",\"code\":\"JAM_DETECTED\",\"dispensed\":" + String(totalDispensed) + "}";
  } else {
    resp = "{\"status\":\"SUCCESS\",\"dispensed\":" + String(totalDispensed) + "}";
  }

  // Transmisión robusta con Checksum XOR (Protección Anti-Spoofing e Integridad de Trama)
  uint8_t chk = 0;
  for (unsigned int i = 0; i < resp.length(); i++) {
    chk ^= (uint8_t)resp[i];
  }
  char hexBuf[3];
  sprintf(hexBuf, "%02X", chk);
  Serial.print(resp);
  Serial.print('*');
