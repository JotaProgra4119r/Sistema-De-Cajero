...
class BankingService:
    @staticmethod
    def calculate_optimal_dispense(amount: int, vault_stock: Dict[int, int]) -> Optional[Dict[str, int]]:
        """
        Algoritmo de Programación Dinámica y Backtracking con Branch-and-Bound:
        Calcula la combinación exacta de billetes en Quetzales para cualquier monto escalar
        (incluyendo montos impares no estándar como Q123.00, Q239.00), respetando estrictamente
        el inventario físico disponible en la bóveda por denominación.
        
        Prioriza denominaciones mayores para minimizar el conteo total de billetes y utiliza
        memoización de estados para responder en tiempo sub-milisegundo.
        Retorna None si no es físicamente posible dispensar el monto exacto con el stock actual.
        """
        if amount <= 0:
            return {}

        sorted_denoms = sorted([d for d in VALID_DENOMINATIONS if vault_stock.get(d, 0) > 0], reverse=True)
        memo = {}

        def backtrack(rem: int, idx: int) -> Optional[Dict[int, int]]:
            if rem == 0:
                return {}
            ...
            d = sorted_denoms[idx]
            max_can_use = min(rem // d, vault_stock.get(d, 0))

            for count in range(max_can_use, -1, -1):
                sub_res = backtrack(rem - count * d, idx + 1)
    ...
    async def withdraw_custom(db: Session, user_id: int, amount: float, bills: Dict[str, int]) -> Dict[str, Any]:
        ...
        # 3. Determinación y consistencia de desglose físico
        vault_records = {d.denominacion: d for d in db.query(DenominacionCajero).with_for_update().all()}
        stock_map = {d.denominacion: d.cantidad_billetes for d in vault_records.values()}

        clean_bills = {}
        if not bills or sum(bills.values()) == 0:
            optimal = BankingService.calculate_optimal_dispense(int(amount), stock_map)
            if not optimal:
                raise HTTPException(
                    status_code=400,
                    detail=f"Inventario físico insuficiente: No es posible dispensar exactamente Q{amount:.2f} con los billetes disponibles en bóveda."
                )
            clean_bills = optimal
        else:
            sum_selected = 0
    ...
    async def soft_delete_user(db: Session, admin_user_id: int, user_id: int, reason: str = "Baja solicitada por administración") -> Dict[str, Any]:
        ...
        await txt_manager.append_auditoria_eliminacion(
            user_id, admin_user_id, reason, snapshot, now
        )
